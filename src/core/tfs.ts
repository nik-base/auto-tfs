...
export class Tfs {
  ...
  public shelve(sourceControl: SourceControl): void {
    ...
    this.execWithoutConfirm(command, uriList, {
      name: shelveName,
  ...
  public checkin(sourceControl: SourceControl): void {
    if (
      !this.configuration.tfCheckin() ||
      this.configuration.tfCheckin() === 'Disabled'
    ) {
      return;
    }
    const uriList = this.getIncludedChanges();
    ...
    this.execWithData(
      command,
      uriList,
      { comment: comment, sourceControl: sourceControl },
  ...
  public async getAll(): Promise<void> {
    ...
    SCM.startGetAll();
    const command = new GetTfsCommand();
    await this.executeCommandSyncWithConfirm(
      command,
      [root],
      null,
      this.confirmGet.bind(this)
    ).then(() => {
  ...
  public async scmOpen(uri: Uri, change: SCMChange): Promise<void> {
    ...
    if (
      change.type === SCMChangeType.Renamed ||
      SCMChangeType.RenamedModified
    ) {
      const sourceItem = this.getSourceItem(uri);
      const command = new ViewTfsCommand();
      const parsedTemp = pathParse(uri.fsPath);
      this.executeCommand(command, [uri], {
        temp: parsedTemp,
  ...
  public rename(
    files: ReadonlyArray<{ readonly oldUri: Uri; readonly newUri: Uri }>
  ): Promise<void> {
    if (!files.length) {
  ...
  public confirmShelveReplace(shelve: string): Thenable<string | undefined> {
    return this.message.warning(
      `Shelve '${shelve}' already exists. Do you want to replace it?`,
      'Yes',
      'No'
    );
  }
  ...
  private getSourceItem(uri: Uri): string | null {
    ...
    const result = this.executeCommandSync(command, [uri], {
      item: 'sourceitem',
    });
    const sourceItem = new InfoProcessHandler().getSourceItem(
      result!.toString()
    );
    return sourceItem;
  }

  private execWithoutConfirm(
    command: TfsCommand,
    uriList: readonly Uri[],
    data: any
  ): void {
    if (!uriList.length) {
      return;
  ...
  private exec(
    command: TfsCommand,
    uriList: readonly Uri[],
    confirm: () => Thenable<string | undefined>
  ): void {
    if (!uriList.length) {
      return;
    }
  ...
  private async executeCommandSyncWithConfirm(
    command: TfsCommand,
    uriList: readonly Uri[],
    data: any,
    confirm: () => Thenable<string | undefined>
  ...
  private getTfPath(): string | null {
    ...
    const message =
      'The path to TF command is not configured. Please, check the property auto-tfs.tf.path in VS Code settings';
  ...
  private isShellRequired(command: TfsCommand): boolean {
    ...
    if (
      ...
      this.configuration.tfCheckin() === 'With Prompt'
    ) {
      return true;
    }
  ...
  private checkInfo(
    ...
    const processHandler =
      infoCommand.getConsoleDataHandler() as InfoProcessHandler;
